...
public class ConfigInterceptor extends HandlerInterceptorAdapter {
    @Override
    //Thêm thông tin vào modelAndView sau khi xử lý request
    public void postHandle(HttpServletRequest request, HttpServletResponse response, Object handler, ModelAndView modelAndView) throws Exception {
        Authentication xacthuc = SecurityContextHolder.getContext().getAuthentication();
        //ktra đăng nhập (trả veef false hoặc true
        if (!xacthuc.getAuthorities().contains(new SimpleGrantedAuthority("ROLE_ANONYMOUS"))) {
            CustomUserDetails principal = (CustomUserDetails) xacthuc.getPrincipal();
